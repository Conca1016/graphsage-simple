...
class SupervisedGraphSage(nn.Module):
    ...
    def __init__(self, emb_dim, hls_dim, enc):
        ...
        self.weight1 = nn.Parameter(torch.FloatTensor(8, self.total_dim))
        init.normal_(self.weight1, mean=0, std=1)
        self.bn1 = nn.BatchNorm1d(8)      
  
        self.weight2 = nn.Parameter(torch.FloatTensor(1, 8))
...
def graph_forward(node_feat_dim, features, adj_lists0, adj_lists1, agg_sel=False):
    ...
    enc1 = Encoder(features, node_feat_dim, 6, adj_lists0, adj_lists1, agg1_0, agg1_1, gcn=agg_sel, cuda=False)

    agg2_0 = MeanAggregator(lambda nodes : enc1(nodes).t(), cuda=False)
    agg2_1 = MeanAggregator(lambda nodes : enc1(nodes).t(), cuda=False)
    enc2 = Encoder(lambda nodes : enc1(nodes).t(), enc1.embed_dim, 5, adj_lists0, adj_lists1, agg2_0, agg2_1,
    ...
    enc3 = Encoder(lambda nodes : enc2(nodes).t(), enc2.embed_dim, 5, adj_lists0, adj_lists1, agg3_0, agg3_1,
